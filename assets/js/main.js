...
let myH1Text = document.createTextNode('Zufälliges Bild mit JavaScript anzeigen')
myH1.appendChild(myH1Text)
document.getElementById('content').appendChild(myH1)

let myButton = document.createElement('button')
let myButtonText = document.createTextNode('Click me')
...
document.querySelector('button').addEventListener('click', () => {
    let zuf = Math.floor(Math.random() * dataLength) + 1
    // console.log(zuf)
    // console.log(data[zuf].download_url)
    if (i == 1) {
        myImg.remove()
    }
    myImg = document.createElement('img')
    myImg.src = data[zuf].download_url
